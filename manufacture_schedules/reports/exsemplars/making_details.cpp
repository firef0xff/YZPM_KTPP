...
struct CalcData
{
    ...
    void inc(const MakingDetails::Data &r)
    {
        req+=r.trud_req;
        mk+=r.trud_maked;
        br+=r.trud_broken;
...
void MakingDetails::BuildReport()
{
    ...
    DB->SendCommand("create temporary table if not exists tmp2 "
        ...
        "key utch(`utch`) "
        ")engine = MEMORY");
    DB->SendCommand("insert into tmp2 (request,maked,broken,unmaked,cex,utch,trud_request,trud_maked,trud_broken,trud_unmaked,det_id,vz) "
        "select "
        "`a`.`request`, "
        "min(`a`.`maked`)  maked, "
        "max(`a`.`broken`) broken, "
        ...
        "group by `b`.`cex`,`b`.`utch`,`d`.`det_id`");
    ...
    if (rez)
    {
        if (rez->RecordCount)
        {
            ...
            size_t max_rows = end_row - start_row;
            size_t cur_rows(0);
            size_t list_count(0);
